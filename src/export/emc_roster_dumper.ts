...
async function queryPlayerHistory(players: { firstName: string, lastName: string }[]) {
  ...
  try {
    ...
    for (const player of players) {
      ...
      // Query players matching firstName and lastName /madden_data27/3418359/MADDEN_PLAYER/551029616/history/285facce-919d-4d41-8771-4a9405d669e1
      const playerQuery = await db
        .collection('madden_data27').doc("3418359").collection("MADDEN_PLAYER")
      ...
      for (const doc of playerQuery.docs) {
        ...
        const historyRef = db.collection(`madden_data27/3418359/MADDEN_PLAYER/${doc.id}/history`);
